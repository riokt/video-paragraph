...
class DecoderLayer(nn.Module):
    ...
    def forward(self, inputs, src_mask, trg_mask):
        ...
        y, attn = self.multi_head_attention_enc_dec(x_norm, encoder_outputs, encoder_outputs, src_mask, is_dec=True)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, src_mask=None, is_dec=False):
        ...
        if src_mask is not None:
            ...
            for i in range(logits.shape[1]):
                print(i)
                logits[:,i,:,:] = logits[:,i,:,:].unsqueeze(1).masked_fill(src_mask == 0, -1e9)
